security {
	saml {
		...
		userAttributeMappings = [:] // Map holding various 'Name' attributes inside 'saml2:Attribute' elements. Key called 'username' is used to map Attribute holding username. Other keys are equal to fields in Spring Security user class and are mapped from 'saml2:Attribute' elements.
		userGroupAttribute = "memberOf" // Value of 'Name' attribute inside 'saml2:Attribute' element where the user's groups are received. Eg.: 'http://wso2.org/claims/role'. It expects the group strings inside 'saml2:Attribute' to be of the format: "CN=groupName,someOtherParam=someOtherValue"
